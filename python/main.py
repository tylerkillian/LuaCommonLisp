...
def test_ConsReader2_emptyList():
	...
	shouldBeSpace = reader.readNextCharacter(readerStack, " ")
...
def test_ConsReader2_singleElementList():
	...
	shouldBeNull = consReader.readNextCharacter(readerStack, "a")
	...
	shouldBeNull = consReader.readNextCharacter(readerStack, ")")
	assert(not shouldBeNull)
	assert(not consReader.isDone())
	shouldBeSpace = consReader.readNextCharacter(readerStack, " ")
	assert(shouldBeSpace == " ")
